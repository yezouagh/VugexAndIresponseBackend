...
public class InstancesManager extends Thread {
    ...
    public void run() {
        try {
            if (!this.account.getEmpty()) {
                ...
                vltrMng.setAccount(this.account);
                ...
                String instanceId = vltrMng.createInstance(hostName, this.process.region, this.process.os, this.process.size);
                if (instanceId == null){
                    throw new DatabaseException("Vultr Instance could not be created !");
                }
                ThreadSleep.sleep(120000L);
                HashMap instanceInfo = vltrMng.getInstanceById(instanceId);
                if (instanceInfo == null){
                    throw new DatabaseException("Vultr Instance could not be created !");
                }
                Vultr.updateCountInstanceCreated(1);
                UpdateData.updateInstancesCreated(this.process);
                VultrInstance vltrInstance = new VultrInstance();
                vltrInstance.status = "Running";
                vltrInstance.accountId = this.account.id;
                vltrInstance.accountName = this.account.name;
                vltrInstance.name = "mail." + domin.value + " (" + instanceId + ")";
                vltrInstance.instanceId = instanceId;
                vltrInstance.region = this.process.region;
                vltrInstance.platform = this.process.os;
                vltrInstance.size = this.process.size;
                MtaServer mtaServ = getServerInfos(this.account, instanceInfo, this.provider, domin);
                vltrInstance.mtaServerId = mtaServ.id;
                vltrInstance.mtaServerName = mtaServ.name;
                vltrInstance.createdBy = (Application.checkAndgetInstance().getUser()).email;
                vltrInstance.createdDate = new Date(System.currentTimeMillis());
                vltrInstance.lastUpdatedBy = (Application.checkAndgetInstance().getUser()).email;
                vltrInstance.lastUpdatedDate = new Date(System.currentTimeMillis());
                vltrInstance.insert();
                SSHConnector ssh = null;
                try {
                    ssh = Authentification.connectToServer(mtaServ);
                    if (ssh == null || !ssh.isConnected()){
                        throw new DatabaseException("Could not connect to this Vultr Instance : " + mtaServ.name + " !");
                    }
                    ...
                    mtaServ.update();
                    Vultr.updateCountInstanceInstalled();
                } finally {
                    if (ssh != null){
                        ssh.disconnect();
                    }
            ...
            Loggers.error(th);
            Vultr.IS_ERROR_OCCURED = true;
        }
    }

    public static synchronized MtaServer getServerInfos(VultrAccount vltrAcc, HashMap instanceInfo, ServerProvider servProvider, Domain domin) throws Exception {
        String name = vltrAcc.name.trim();
        String nm = name + "_1";
    ...
    public InstancesManager(VultrAccount account, VultrProcess process, ServerProvider provider, String domainId) {
    ...
    public boolean equals(Object paramObject) {
        ...
        VultrAccount const2 = if1.getAccount();
        if ((const1 == null) ? (const2 != null) : !const1.equals(const2))
            return false;
        VultrProcess float1 = getProcess();
        VultrProcess float2 = if1.getProcess();
    ...
    public int hashCode() {
        int n = 1;
        VultrAccount char1 = getAccount();
        n = n * 59 + ((char1 == null) ? 43 : char1.hashCode());
        VultrProcess goto1 = getProcess();
    ...
    public VultrProcess getProcess() {
        return process;
    }

    public void setProcess(VultrProcess process) {
        this.process = process;
    }
